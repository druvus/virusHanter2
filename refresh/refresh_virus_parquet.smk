...
# The searchable databases the `viral_rna_mito` alias bundles: every
# configured BLAST DB except taxdb (a taxonomy-lookup helper, not a
# sequence DB). Deriving this from BLAST_NAMES keeps the alias in step
# with what was actually downloaded.
BLAST_ALIAS_DBS = [name for name in BLAST_NAMES if name != "taxdb"]
...
#
# The `viral_rna_mito.nal` alias bundles every searchable DB in
# BLAST_NAMES (all except taxdb) so a single BLAST run queries them
# together. `taxdb` is decompressed in-place so BLAST's subject-title
# resolution works at run time.
rule refresh_blast:
    ...
    params:
        ...
        alias_dbs=" ".join(BLAST_ALIAS_DBS),
    log:
    ...
    shell:
        ...
        # Write the alias file (.nal) so a single BLAST invocation
        # queries every configured searchable DB (the production
        # `viral_rna_mito` alias the main pipeline points at). DBLIST
        # is derived from BLAST_NAMES (minus taxdb), so it always
        # matches what was actually downloaded.
        cat > {output.alias} <<NAL
#
# BLAST alias bundling the configured viral RefSeq reference genomes
# (and any additional references listed in BLAST_NAMES). Regenerated
# by the refresh workflow whenever the underlying BLAST tarballs change.
#
TITLE viral_rna_mito
DBLIST {params.alias_dbs}
NAL
